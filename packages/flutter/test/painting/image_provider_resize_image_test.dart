  ...
  group('ResizeImage', () {
    group('resizing', () {
      test('upscales to the correct dimensions', () async {
        ...
        final resizedImage = ResizeImage(MemoryImage(bytes), width: resizeDims.width.round(), height: resizeDims.height.round(), allowUpscaling: true);
      ...
      test('refuses upscaling when allowUpscaling=false', () async {
        ...
        final resizedImage = ResizeImage(MemoryImage(bytes), width: resizeDims.width.round(), height: resizeDims.height.round());
        const resizeConfig = ImageConfiguration(size: resizeDims);
        final Size resizedImageSize = await _resolveAndGetSize(resizedImage, configuration: resizeConfig);
      ...
      group('with policy=fit and allowResize=false', () {
        test('constrains square image to bounded portrait rect', () async {
          ...
          final resizedImage = ResizeImage(rawImage, width: 25, height: 50, policy: ResizeImagePolicy.fit);
        ...
        test('constrains square image to bounded landscape rect', () async {
          final rawImage = MemoryImage(Uint8List.fromList(kBlueSquarePng));
          await _expectImageSize(rawImage, const Size(50, 50));
          final resizedImage = ResizeImage(rawImage, width: 50, height: 25, policy: ResizeImagePolicy.fit);
          await _expectImageSize(resizedImage, const Size(25, 25));
        });
        ...
        test('constrains square image to bounded square', () async {
          final rawImage = MemoryImage(Uint8List.fromList(kBlueSquarePng));
          await _expectImageSize(rawImage, const Size(50, 50));
          final resizedImage = ResizeImage(rawImage, width: 25, height: 25, policy: ResizeImagePolicy.fit);
          await _expectImageSize(resizedImage, const Size(25, 25));
        });
        ...
        test('constrains portrait image to bounded portrait rect', () async {
          ...
          final resizedImage = ResizeImage(rawImage, width: 25, height: 60, policy: ResizeImagePolicy.fit);
        ...
        test('constrains portrait image to bounded landscape rect', () async {
          final rawImage = MemoryImage(Uint8List.fromList(kBluePortraitPng));
          await _expectImageSize(rawImage, const Size(50, 100));
          final resizedImage = ResizeImage(rawImage, width: 60, height: 25, policy: ResizeImagePolicy.fit);
        ...
        test('leaves image as-is if constraints are bigger than image', () async {
          final rawImage = MemoryImage(Uint8List.fromList(kBlueSquarePng));
          await _expectImageSize(rawImage, const Size(50, 50));
          final resizedImage = ResizeImage(rawImage, width: 120, height: 100, policy: ResizeImagePolicy.fit);
      ...
      group('with policy=fit and allowResize=true', () {
        test('constrains square image to bounded portrait rect', () async {
          final rawImage = MemoryImage(Uint8List.fromList(kBlueSquarePng));
          await _expectImageSize(rawImage, const Size(50, 50));
          final resizedImage = ResizeImage(rawImage, width: 100, height: 200, policy: ResizeImagePolicy.fit, allowUpscaling: true);
        ...
        test('constrains square image to bounded landscape rect', () async {
          final rawImage = MemoryImage(Uint8List.fromList(kBlueSquarePng));
          await _expectImageSize(rawImage, const Size(50, 50));
          final resizedImage = ResizeImage(rawImage, width: 200, height: 100, policy: ResizeImagePolicy.fit, allowUpscaling: true);
          await _expectImageSize(resizedImage, const Size(100, 100));
        });
        ...
        test('constrains square image to bounded square', () async {
          final rawImage = MemoryImage(Uint8List.fromList(kBlueSquarePng));
          await _expectImageSize(rawImage, const Size(50, 50));
          final resizedImage = ResizeImage(rawImage, width: 100, height: 100, policy: ResizeImagePolicy.fit, allowUpscaling: true);
          await _expectImageSize(resizedImage, const Size(100, 100));
        });
        ...
        test('constrains square image to bounded width', () async {
          final rawImage = MemoryImage(Uint8List.fromList(kBlueSquarePng));
          await _expectImageSize(rawImage, const Size(50, 50));
          final resizedImage = ResizeImage(rawImage, width: 100, policy: ResizeImagePolicy.fit, allowUpscaling: true);
          await _expectImageSize(resizedImage, const Size(100, 100));
        });
        ...
        test('constrains square image to bounded height', () async {
          final rawImage = MemoryImage(Uint8List.fromList(kBlueSquarePng));
          await _expectImageSize(rawImage, const Size(50, 50));
          final resizedImage = ResizeImage(rawImage, height: 100, policy: ResizeImagePolicy.fit, allowUpscaling: true);
          await _expectImageSize(resizedImage, const Size(100, 100));
        });
        ...
        test('constrains portrait image to bounded portrait rect', () async {
          final rawImage = MemoryImage(Uint8List.fromList(kBluePortraitPng));
          await _expectImageSize(rawImage, const Size(50, 100));
          final resizedImage = ResizeImage(rawImage, width: 100, height: 250, policy: ResizeImagePolicy.fit, allowUpscaling: true);
        ...
        test('constrains portrait image to bounded landscape rect', () async {
          final rawImage = MemoryImage(Uint8List.fromList(kBluePortraitPng));
          await _expectImageSize(rawImage, const Size(50, 100));
          final resizedImage = ResizeImage(rawImage, width: 400, height: 200, policy: ResizeImagePolicy.fit, allowUpscaling: true);
          await _expectImageSize(resizedImage, const Size(100, 200));
        });
        ...
        test('constrains portrait image to bounded square', () async {
          final rawImage = MemoryImage(Uint8List.fromList(kBluePortraitPng));
          await _expectImageSize(rawImage, const Size(50, 100));
          final resizedImage = ResizeImage(rawImage, width: 200, height: 200, policy: ResizeImagePolicy.fit, allowUpscaling: true);
          await _expectImageSize(resizedImage, const Size(100, 200));
        });
        ...
        test('constrains portrait image to bounded height', () async {
          final rawImage = MemoryImage(Uint8List.fromList(kBluePortraitPng));
          await _expectImageSize(rawImage, const Size(50, 100));
          final resizedImage = ResizeImage(rawImage, height: 200, policy: ResizeImagePolicy.fit, allowUpscaling: true);
          await _expectImageSize(resizedImage, const Size(100, 200));
        });
        ...
        test('constrains landscape image to bounded portrait rect', () async {
          final rawImage = MemoryImage(Uint8List.fromList(kBlueLandscapePng));
          await _expectImageSize(rawImage, const Size(100, 50));
          final resizedImage = ResizeImage(rawImage, width: 200, height: 400, policy: ResizeImagePolicy.fit, allowUpscaling: true);
        ...
        test('constrains landscape image to bounded landscape rect', () async {
          final rawImage = MemoryImage(Uint8List.fromList(kBlueLandscapePng));
          await _expectImageSize(rawImage, const Size(100, 50));
          final resizedImage = ResizeImage(rawImage, width: 250, height: 100, policy: ResizeImagePolicy.fit, allowUpscaling: true);
          await _expectImageSize(resizedImage, const Size(200, 100));
        });
        ...
        test('constrains landscape image to bounded width', () async {
          final rawImage = MemoryImage(Uint8List.fromList(kBlueLandscapePng));
          await _expectImageSize(rawImage, const Size(100, 50));
          final resizedImage = ResizeImage(rawImage, width: 200, policy: ResizeImagePolicy.fit, allowUpscaling: true);
          await _expectImageSize(resizedImage, const Size(200, 100));
        });
        ...
        test('shrinks image if constraints are smaller than image', () async {
          final rawImage = MemoryImage(Uint8List.fromList(kBlueSquarePng));
          await _expectImageSize(rawImage, const Size(50, 50));
          final resizedImage = ResizeImage(rawImage, width: 25, height: 30, policy: ResizeImagePolicy.fit, allowUpscaling: true);
          await _expectImageSize(resizedImage, const Size(25, 25));
        });
      });
