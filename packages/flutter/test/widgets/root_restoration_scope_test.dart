...
void main() {
  ...
  testWidgets('does not insert root when restoration id is null', (WidgetTester tester) async {
    ...
    // Complete the future.
    final root = RestorationBucket.root(manager: binding.restorationManager, rawData: <String, dynamic>{});
    addTearDown(root.dispose);
    bucketCompleter.complete(root);
    await tester.pump(const Duration(milliseconds: 100));
  ...
  testWidgets('injects root bucket when moved out of scope', (WidgetTester tester) async {
    ...
    final outOfScopeRootBucket = RestorationBucket.root(manager: binding.restorationManager, rawData: outOfScopeRawData);
  ...
  testWidgets('injects new root when old one is decommissioned', (WidgetTester tester) async {
    ...
    final firstRoot = RestorationBucket.root(manager: binding.restorationManager, rawData: firstRawData);
    ...
    final secondRoot = RestorationBucket.root(manager: binding.restorationManager, rawData: secondRawData);
