...
void main() {
  ...
  testWidgets('Image State can be reconfigured to use another image', (WidgetTester tester) async {
    final image1 = Image(image: _TestImageProvider()..complete(image10x10.clone()), width: 10.0, excludeFromSemantics: true);
    final image2 = Image(image: _TestImageProvider()..complete(image10x10.clone()), width: 20.0, excludeFromSemantics: true);
  ...
  (WidgetTester tester) async {
    ...
    final ImageStream stream = provider.resolve(provider._lastResolvedConfiguration);
    late bool isSync;
    final listener = ImageStreamListener((ImageInfo image, bool syncCall) { isSync = syncCall; });
